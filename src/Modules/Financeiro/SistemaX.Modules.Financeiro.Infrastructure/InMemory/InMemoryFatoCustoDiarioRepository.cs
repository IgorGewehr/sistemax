...
public sealed class InMemoryFatoCustoDiarioRepository : IFatoCustoDiarioRepository
{
    ...
    public Task<FatoCustoDiario?> ObterAsync(string tenantId, DateOnly dia, CorrenteDeReceita corrente, CancellationToken ct = default)
        => Task.FromResult(_porChave.TryGetValue((tenantId, dia, corrente), out var valor)
            ? new FatoCustoDiario(tenantId, dia, corrente, valor, DateTimeOffset.UtcNow)
            : null);
    ...
    public Task<IReadOnlyList<FatoCustoDiario>> ListarAsync(string tenantId, DateOnly de, DateOnly ate, CancellationToken ct = default)
        => Task.FromResult<IReadOnlyList<FatoCustoDiario>>(_porChave
            ...
            .Select(kv => new FatoCustoDiario(tenantId, kv.Key.Dia, kv.Key.Corrente, kv.Value, DateTimeOffset.UtcNow))
            .ToList());
