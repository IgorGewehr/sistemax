...
public sealed class InMemoryFatoReceitaDiariaRepository : IFatoReceitaDiariaRepository
{
    ...
    public Task<FatoReceitaDiaria?> ObterAsync(string tenantId, DateOnly dia, CorrenteDeReceita corrente, string projetoId = "", CancellationToken ct = default)
        => Task.FromResult(_porChave.TryGetValue((tenantId, dia, corrente, projetoId), out var valor)
            ? new FatoReceitaDiaria(tenantId, dia, corrente, projetoId, valor, DateTimeOffset.UtcNow)
            : null);
    ...
    public Task<IReadOnlyList<FatoReceitaDiaria>> ListarAsync(string tenantId, DateOnly de, DateOnly ate, CancellationToken ct = default)
        => Task.FromResult<IReadOnlyList<FatoReceitaDiaria>>(_porChave
            ...
            .Select(kv => new FatoReceitaDiaria(tenantId, kv.Key.Dia, kv.Key.Corrente, kv.Key.ProjetoId, kv.Value, DateTimeOffset.UtcNow))
            .ToList());
