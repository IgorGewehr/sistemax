...
/// <summary>Espelha <see cref="SqliteContaAReceberRepository"/> — mesma estratégia de filha
/// mutável (delete + reinsert de <see cref="Parcela"/> dentro da mesma operação), trocando
/// <c>cliente_id</c> por <c>fornecedor_id</c>.</summary>
public sealed class SqliteContaAPagarRepository(ILocalSqliteConnectionFactory connectionFactory, ILocalSessao sessao)
    : IContaAPagarRepository
{
    ...
    /// <summary>Espelha o in-memory: dois <c>Any()</c> independentes — ver comentário em
    /// <see cref="SqliteContaAReceberRepository.ListarAbertasAteAsync"/>.</summary>
        ...
        => ConsultarAsync(async (connection, transaction) =>
        {
            var contas = await LerContasAsync(connection, transaction,
                ...
                WHERE business_id = $biz
                  AND id IN (SELECT conta_id FROM parcelas_a_pagar WHERE status IN ($aberto, $parcial, $atrasado))
                  AND id IN (SELECT conta_id FROM parcelas_a_pagar WHERE vencimento <= $referencia)
                """,
                cmd =>
                {
                    cmd.Parameters.AddWithValue("$biz", businessId);
    ...
    public Task SalvarAsync(ContaAPagar conta, CancellationToken ct = default)
        => ExecutarAsync(async (connection, transaction) =>
        {
            ...
            await using (var delCmd = connection.CreateCommand())
            {
                ...
                delCmd.CommandText = "DELETE FROM parcelas_a_pagar WHERE conta_id = $contaId;";
    ...
    private static async Task<List<ContaAPagar>> LerContasAsync(
        ...
        Action<SqliteCommand> configurarParametros, CancellationToken ct)
    {
        ...
        {
            cmd.Transaction = transaction;
            cmd.CommandText = $"SELECT {ColunasHeader} FROM contas_a_pagar {whereClause};";
        ...
        var resultado = new List<ContaAPagar>(headers.Count);
