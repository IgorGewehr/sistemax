    ...
    : IFatoReceitaDiariaRepository
{
    ...
    public Task<IReadOnlyList<FatoReceitaDiaria>> ListarAsync(string tenantId, DateOnly de, DateOnly ate, CancellationToken ct = default)
        => ConsultarAsync(async (connection, transaction) =>
        {
            ...
            var resultado = new List<FatoReceitaDiaria>();
            await using var reader = await cmd.ExecuteReaderAsync(ct).ConfigureAwait(false);
            while (await reader.ReadAsync(ct).ConfigureAwait(false))
            {
                resultado.Add(Ler(reader));
            }
            return (IReadOnlyList<FatoReceitaDiaria>)resultado;
        }, ct);
    ...
    public Task ZerarTudoAsync(CancellationToken ct = default)
        => ExecutarAsync(async (connection, transaction) =>
        {
            await using var cmd = connection.CreateCommand();
            cmd.Transaction = transaction;
            cmd.CommandText = "DELETE FROM fato_receita_diaria;";
            await cmd.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
        }, ct);

    private static FatoReceitaDiaria Ler(SqliteDataReader reader)
        => new(
