...
/// </summary>
public sealed class SqliteMovimentoRepositoryContractTests : MovimentoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-movimento-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteMovimentoRepositoryContractTests()
    {
        ...
        new EstoqueSchemaMigrationV3().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IMovimentoRepository CriarRepositorio()
        => new SqliteMovimentoRepository(_connectionFactory, new SessaoSempreInativa());
