...
public sealed class SqliteApontamentoDeTempoRepositoryContractTests : ApontamentoDeTempoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-apontamento-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteApontamentoDeTempoRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV36().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IApontamentoDeTempoRepository CriarRepositorio()
        => new SqliteApontamentoDeTempoRepository(_connectionFactory, new SessaoSempreInativa());
