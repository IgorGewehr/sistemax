...
public sealed class SqliteAtivoDeCapitalRepositoryContractTests : AtivoDeCapitalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-ativo-capital-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteAtivoDeCapitalRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV35().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IAtivoDeCapitalRepository CriarRepositorio()
        => new SqliteAtivoDeCapitalRepository(_connectionFactory, new SessaoSempreInativa());
