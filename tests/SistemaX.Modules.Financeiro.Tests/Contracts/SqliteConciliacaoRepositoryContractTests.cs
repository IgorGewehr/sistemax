...
public sealed class SqliteConciliacaoRepositoryContractTests : ConciliacaoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-conciliacao-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteConciliacaoRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV5().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IConciliacaoRepository CriarRepositorio()
        => new SqliteConciliacaoRepository(_connectionFactory, new SessaoSempreInativa());
