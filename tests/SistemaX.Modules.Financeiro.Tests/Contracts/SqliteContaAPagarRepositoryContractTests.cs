...
public sealed class SqliteContaAPagarRepositoryContractTests : ContaAPagarRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-conta-a-pagar-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteContaAPagarRepositoryContractTests()
    {
    ...
    protected override IContaAPagarRepository CriarRepositorio()
        => new SqliteContaAPagarRepository(_connectionFactory, new SessaoSempreInativa());
