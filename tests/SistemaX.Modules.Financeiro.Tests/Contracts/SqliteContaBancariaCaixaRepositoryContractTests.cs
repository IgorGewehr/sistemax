...
public sealed class SqliteContaBancariaCaixaRepositoryContractTests : ContaBancariaCaixaRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-conta-bancaria-caixa-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteContaBancariaCaixaRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV12().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IContaBancariaCaixaRepository CriarRepositorio()
        => new SqliteContaBancariaCaixaRepository(_connectionFactory, new SessaoSempreInativa());
