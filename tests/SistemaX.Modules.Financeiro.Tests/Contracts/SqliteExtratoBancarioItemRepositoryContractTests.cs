...
public sealed class SqliteExtratoBancarioItemRepositoryContractTests : ExtratoBancarioItemRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-extrato-bancario-item-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteExtratoBancarioItemRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV6().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IExtratoBancarioItemRepository CriarRepositorio()
        => new SqliteExtratoBancarioItemRepository(_connectionFactory, new SessaoSempreInativa());
