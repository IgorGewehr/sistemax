...
public sealed class SqliteFatoCaixaDiarioRepositoryContractTests : FatoCaixaDiarioRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-fato-caixa-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFatoCaixaDiarioRepositoryContractTests()
    {
    ...
    protected override IFatoCaixaDiarioRepository CriarRepositorio()
        => new SqliteFatoCaixaDiarioRepository(_connectionFactory, new SessaoSempreInativa());
