...
public sealed class SqliteFatoCustoDiarioRepositoryContractTests : FatoCustoDiarioRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-fato-custo-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFatoCustoDiarioRepositoryContractTests()
    {
    ...
    protected override IFatoCustoDiarioRepository CriarRepositorio()
        => new SqliteFatoCustoDiarioRepository(_connectionFactory, new SessaoSempreInativa());
