...
public sealed class SqliteFatoMargemProdutoRepositoryContractTests : FatoMargemProdutoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-fato-margem-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFatoMargemProdutoRepositoryContractTests()
    {
    ...
    protected override IFatoMargemProdutoRepository CriarRepositorio()
        => new SqliteFatoMargemProdutoRepository(_connectionFactory, new SessaoSempreInativa());
