...
public sealed class SqliteFatoRecebiveisRepositoryContractTests : FatoRecebiveisRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-fato-recebiveis-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFatoRecebiveisRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV11().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IFatoRecebiveisRepository CriarRepositorio()
        => new SqliteFatoRecebiveisRepository(_connectionFactory, new SessaoSempreInativa());
