...
public sealed class SqliteFormaDePagamentoRepositoryContractTests : FormaDePagamentoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-forma-pagamento-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFormaDePagamentoRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV13().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IFormaDePagamentoRepository CriarRepositorio()
        => new SqliteFormaDePagamentoRepository(_connectionFactory, new SessaoSempreInativa());
