...
public sealed class SqliteLancamentoContabilRepositoryContractTests : LancamentoContabilRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-lancamento-contabil-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteLancamentoContabilRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV4().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override ILancamentoContabilRepository CriarRepositorio()
        => new SqliteLancamentoContabilRepository(_connectionFactory, new SessaoSempreInativa());
