...
public sealed class SqliteMovimentoFinanceiroRepositoryContractTests : MovimentoFinanceiroRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-movimento-financeiro-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteMovimentoFinanceiroRepositoryContractTests()
    {
    ...
    protected override IMovimentoFinanceiroRepository CriarRepositorio()
        => new SqliteMovimentoFinanceiroRepository(_connectionFactory, new SessaoSempreInativa());
