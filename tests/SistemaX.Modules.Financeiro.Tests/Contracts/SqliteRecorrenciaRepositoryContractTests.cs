...
public sealed class SqliteRecorrenciaRepositoryContractTests : RecorrenciaRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-recorrencia-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteRecorrenciaRepositoryContractTests()
    {
    ...
    protected override IRecorrenciaRepository CriarRepositorio()
        => new SqliteRecorrenciaRepository(_connectionFactory, new SessaoSempreInativa());
