...
public sealed class SqliteSessaoCaixaRepositoryContractTests : SessaoCaixaRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-sessao-caixa-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteSessaoCaixaRepositoryContractTests()
    {
        ...
        new FinanceiroSchemaMigrationV14().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override ISessaoCaixaRepository CriarRepositorio()
        => new SqliteSessaoCaixaRepository(_connectionFactory, new SessaoSempreInativa());
