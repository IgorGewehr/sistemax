...
public sealed class SqliteCadastroFiscalEmitenteRepositoryContractTests : CadastroFiscalEmitenteRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-cadastro-emitente-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteCadastroFiscalEmitenteRepositoryContractTests()
    {
    ...
    protected override ICadastroFiscalEmitenteRepository CriarRepositorio()
        => new SqliteCadastroFiscalEmitenteRepository(_connectionFactory, new SessaoSempreInativa());
