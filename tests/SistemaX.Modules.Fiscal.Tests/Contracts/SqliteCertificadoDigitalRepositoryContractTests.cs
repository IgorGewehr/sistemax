...
public sealed class SqliteCertificadoDigitalRepositoryContractTests : CertificadoDigitalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-certificado-digital-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteCertificadoDigitalRepositoryContractTests()
    {
    ...
    protected override ICertificadoDigitalRepository CriarRepositorio()
        => new SqliteCertificadoDigitalRepository(_connectionFactory, new SessaoSempreInativa());
