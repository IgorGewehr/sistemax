...
public sealed class SqliteDestinatarioDocumentoFiscalRepositoryContractTests : DestinatarioDocumentoFiscalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-destinatario-documento-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteDestinatarioDocumentoFiscalRepositoryContractTests()
    {
    ...
    protected override IDestinatarioDocumentoFiscalRepository CriarRepositorio()
        => new SqliteDestinatarioDocumentoFiscalRepository(_connectionFactory, new SessaoSempreInativa());
