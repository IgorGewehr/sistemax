...
public sealed class SqliteDocumentoFiscalRepositoryContractTests : DocumentoFiscalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-documento-fiscal-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteDocumentoFiscalRepositoryContractTests()
    {
    ...
    protected override IDocumentoFiscalRepository CriarRepositorio()
        => new SqliteDocumentoFiscalRepository(_connectionFactory, new SessaoSempreInativa());
