...
public sealed class SqliteFormaPagamentoDocumentoFiscalRepositoryContractTests : FormaPagamentoDocumentoFiscalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-forma-pagamento-documento-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteFormaPagamentoDocumentoFiscalRepositoryContractTests()
    {
    ...
    protected override IFormaPagamentoDocumentoFiscalRepository CriarRepositorio()
        => new SqliteFormaPagamentoDocumentoFiscalRepository(_connectionFactory, new SessaoSempreInativa());
