...
public sealed class SqlitePerfilFiscalNcmRepositoryContractTests : PerfilFiscalNcmRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-perfil-fiscal-ncm-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqlitePerfilFiscalNcmRepositoryContractTests()
    {
    ...
    protected override IPerfilFiscalNcmRepository CriarRepositorio()
        => new SqlitePerfilFiscalNcmRepository(_connectionFactory, new SessaoSempreInativa());
