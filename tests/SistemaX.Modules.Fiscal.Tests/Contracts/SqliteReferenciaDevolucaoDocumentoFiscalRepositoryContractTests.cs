...
public sealed class SqliteReferenciaDevolucaoDocumentoFiscalRepositoryContractTests : ReferenciaDevolucaoDocumentoFiscalRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-referencia-devolucao-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteReferenciaDevolucaoDocumentoFiscalRepositoryContractTests()
    {
        ...
        // fiscal_referencias_devolucao_documento só existe a partir da V2 (gap #5, emissao-mapping.md §11).
        new FiscalSchemaMigrationV2().AplicarAsync(connection, transaction, CancellationToken.None).GetAwaiter().GetResult();
        transaction.Commit();
    }

    protected override IReferenciaDevolucaoDocumentoFiscalRepository CriarRepositorio()
        => new SqliteReferenciaDevolucaoDocumentoFiscalRepository(_connectionFactory, new SessaoSempreInativa());
