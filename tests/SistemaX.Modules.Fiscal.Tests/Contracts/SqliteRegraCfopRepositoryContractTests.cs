...
public sealed class SqliteRegraCfopRepositoryContractTests : RegraCfopRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-regra-cfop-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteRegraCfopRepositoryContractTests()
    {
    ...
    protected override IRegraCfopRepository CriarRepositorio()
        => new SqliteRegraCfopRepository(_connectionFactory, new SessaoSempreInativa());
