...
public sealed class SqliteRegraFiscalPorOperacaoRepositoryContractTests : RegraFiscalPorOperacaoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-regra-fiscal-operacao-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteRegraFiscalPorOperacaoRepositoryContractTests()
    {
    ...
    protected override IRegraFiscalPorOperacaoRepository CriarRepositorio()
        => new SqliteRegraFiscalPorOperacaoRepository(_connectionFactory, new SessaoSempreInativa());
