...
public sealed class SqliteTributacaoProdutoRepositoryContractTests : TributacaoProdutoRepositoryContractTests, IDisposable
{
    private readonly string _dbPath = Path.Combine(Path.GetTempPath(), $"sistemax-tributacao-produto-contract-{Guid.NewGuid():N}.db");
    private readonly LocalSqliteConnectionFactory _connectionFactory;

    public SqliteTributacaoProdutoRepositoryContractTests()
    {
    ...
    protected override ITributacaoProdutoRepository CriarRepositorio()
        => new SqliteTributacaoProdutoRepository(_connectionFactory, new SessaoSempreInativa());
